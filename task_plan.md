...
模块 2-7、阶段 A 与阶段 B 已完成当前地基范围。下一步进入模块 8 单局快照与存档恢复，不提前实现原创剧本业务。
...
| 4. 牌桌内行动 | 行动定义领域字段、候选 / 计划、请求复核、运行实例、进度、结算和行动快照。 | **4.1-4.5 已完成当前范围。** | 模块 1-3。 | 行动实例只属于当前牌桌；不恢复模板式全局制作管理器。 |
...
| 6. 牌桌内战斗与 EX-GAS | 参战关系、阵型、角色卡 ASC、实时能力调用和结算。 | **6.1-6.4 已完成当前地基切片。** 战斗快照明确排除，复杂战斗数值规则暂不实现。 | 模块 5。 | GAS 是能力、属性和效果真相；`Battle` 是牌桌内对象，不成为模板式全局战斗总管。 |
...
| 8. 单局快照与存档恢复 | 非战斗单局、牌桌、行动、任务、角色长期 GAS 状态和当前内容集合的真实快照与恢复。 | 局部候选快照存在，正式存档未开始；活动战斗不进入存档。 | 阶段 B。 | 快照来自运行时唯一状态，不保存 Unity 视图、战斗进度或重复业务数据。 |
| 9. 作者工具与关卡编辑支撑 | 内容校验、引用选择、剧本/牌桌作者工具与可扩展编辑入口。 | 内容校验候选存在，未完成一级验收。 | 模块 7-8 的正式数据。 | 工具不要求策划维护可推导内部 key，也不另造作者源。 |
| **阶段 C：StackCraft 完整等价验收** | 对照模板功能矩阵验证所有“保留/改造吸收”功能，覆盖运行流程、UI、存档和作者工具。 | 模块 7-9 完成后执行；通过后才能称为模板功能吸收完成。 | 阶段 A-B、模块 7-9。 | 必须提交已复现、明确排除、仍缺失三张清单；仍缺失项存在时不得宣称完成。 |
| 10. 联机与 Mod 边界接入 | 控制权、命令、可见性、权威随机、内容包依赖、版本和恢复协议。 | 仅有进程级 Mod 生命周期和架构约束，未实现业务协议。 | 阶段 C 形成稳定领域边界。 | 不以单机全权调用、全局可见状态或裸随机为正式假设。 |

模板经济、卡包、交易和具体生存世界规则不属于当前地基模块；只有用户确认相应业务后才单独立项。
...
5. 用 `FoundationTest` 复现本模块明确选择吸收的玩家可见能力，并用自动化测试覆盖其公开契约。
...
4. **阶段 C：完整模板等价。** 模块 7-9 完成后，对照功能矩阵验收 UI、存档、作者工具和全部选定功能；只有此阶段通过，才能称为 StackCraft 功能吸收完成。
...
| 6. 牌桌内战斗与 EX-GAS | 6.1 参战关系与阵型；6.2 角色卡唯一 ASC；6.3 实时能力调用与结算；6.4 权威随机与存档排除边界。 | **6.1-6.4 已完成当前地基切片。** `Tabletop` 为每场战斗及每次 Ability 激活提供可复现权威随机种子；战斗不存档；测试攻击关闭闪避和暴击，只验证固定基础伤害链。 |
...
| 8. 单局快照与存档恢复 | 8.1 内容集合版本事实；8.2 剧本 / 牌桌 / 行动快照；8.3 角色 GAS 快照；8.4 文件存档与恢复。 | 不保存 Unity 视图、资源句柄或重复业务数据。 |
| 9. 作者工具与关卡编辑支撑 | 9.1 内容校验；9.2 剧本作者工具；9.3 牌桌 / 关卡作者工具；9.4 可扩展编辑入口。 | 工具只能编辑正式作者源，不能制造第二套 key、配置表或运行时真相。 |
| 阶段 C | C.1 完整功能矩阵；C.2 UI / 存档 / 作者工具真实流程；C.3 缺失与排除清单。 | 通过后才能声明模板吸收完成。 |
| 10. 联机与 Mod 边界接入 | 10.1 内容包依赖与版本；10.2 权限、命令与可见性；10.3 权威随机；10.4 同步、恢复和 Mod API。 | 不因预留联机 / Mod 而建立单机第二真相；当前只保留边界，不提前做业务协议。 |
...
- 进程级 `ContentRegistrySystem` 已在 2.1a 删除。`ScenarioDirector` 开局时通过 `ResourceSystem` 解析默认包与已启用 Mod 包，成功构建 `ContentIndex` 后才发布 `ScenarioRun`；当前尚未实现剧本级包依赖、覆盖或热切换协议。
- 内容资源句柄由创建它的 `ScenarioDirector` 唯一持有并随活动单局释放；`ScenarioRun` 不直接依赖 YooAsset / `ResourceHandle`。运行中卸载 Mod 包如何与活动单局协商属于 10.1 内容包生命周期协议，当前没有卸载调用入口，不在 2.1c 新增锁表或第二状态。
...
| 0.4d 统一回归 | 在 `FoundationTest` 验证实际场景加载、切换、叠加卸载、输入过场锁定和资源后端释放。 | 正式剧本、正式 UI 或 Mod 内容协议。 | 新正式链路复现本模块选择吸收的场景切换能力，且无资源句柄或事件时序残留。 |
...
- [x] 7.2 视觉审计：当前代码版候选选择态与填槽缺员态均通过真实 GameView 审计并形成有序验收图。
...
- [x] 7.4 回合制和即时制真实 Game View 均通过功能性视觉审计；禁用按钮与可点击状态可明确区分。
...
- [ ] 8.3 角色 EX-GAS 长期状态：插件 2.0.4 当前没有运行时 ASC 快照 / 枚举公开门面；不得由 Gameplay 越层读取 ECS Buffer 或复制属性、标签、技能真相。需先建立 EX-GAS 正式能力并验证失败清理。
- [x] 8.3 接口裁决：快照只含 ASC 等级、固有标签、属性基础值、Ability Code / 等级；当前作者配置负责属性结构与钳制，调用方负责按 Code 解析官方 `AbilityConfig`。恢复在新 Cell 初始化时原子完成，瞬时 Ability / GE / 冷却 / 临时标签不保存。
- [ ] 8.4 接入 GameCore 文件槽位与模板一致的存档玩家界面；不得新增第二套 SaveSystem。
