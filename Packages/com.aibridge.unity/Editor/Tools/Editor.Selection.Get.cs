            ...
            bool includeActiveTransform = true)
        {
            return UnityAiBridge.Utils.MainThread.Instance.Run(() =>
            {
                var response = new SelectionData()
                {
                    ...
                    ActiveInstanceID = ObjectIdCompat.GetInstanceId(Selection.activeObject)
#else
...
#if UNITY_6000_3_OR_NEWER
                    response.InstanceIDs = Selection.objects.Select(ObjectIdCompat.GetInstanceId).ToArray();
#else
