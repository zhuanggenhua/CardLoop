...
namespace Gameplay.Editor.Content
{
    ...
    public static class FoundationTestSceneMenu
    {
        ...
        public static void RebuildTestScene()
        {
            ...
            gameManagerObject.AddComponent<ContentRegistrySystem>();
            ScenarioTurnSystem scenarioTurnSystem =
                gameManagerObject.AddComponent<ScenarioTurnSystem>();
            QuestSystem questSystem = gameManagerObject.AddComponent<QuestSystem>();
            ScenarioDirector scenarioDirector = gameManagerObject.AddComponent<ScenarioDirector>();
        ...
        private static void VerifySavedSceneConfig(GameConfig expectedConfig)
        {
            ...
            if (savedGameManager.GetComponent<ContentRegistrySystem>() == null)
            {
                throw new MissingReferenceException(
                    $"保存后的测试场景没有 {nameof(ContentRegistrySystem)}：{ScenePath}");
            }
            ...
            if (savedGameManager.GetComponent<ScenarioTurnSystem>() == null)
            {
                throw new MissingReferenceException(
                    $"保存后的测试场景没有 {nameof(ScenarioTurnSystem)}：{ScenePath}");
            }

            if (savedGameManager.GetComponent<QuestSystem>() == null)
            {
                throw new MissingReferenceException(
                    $"保存后的测试场景没有 {nameof(QuestSystem)}：{ScenePath}");
            }
            ...
            SerializedObject serializedSavedScenarioDirector = new(savedScenarioDirector);
            if (serializedSavedScenarioDirector.FindProperty("m_questSystem")?.objectReferenceValue == null)
            {
                throw new MissingReferenceException(
                    $"保存后的测试场景没有装配剧本到任务的父子引用：{ScenePath}");
            }

            if (serializedSavedScenarioDirector.FindProperty("m_turnSystem")?.objectReferenceValue == null)
            {
                throw new MissingReferenceException(
                    $"保存后的测试场景没有装配剧本到回合的父子引用：{ScenePath}");
            }
        ...
        {
            TurnTimingDefinition timingDefinition =
                AssetDatabase.LoadAssetAtPath<TurnTimingDefinition>(TestTurnTimingPath);
            if (timingDefinition == null)
            {
                timingDefinition = ScriptableObject.CreateInstance<TurnTimingDefinition>();
                AssetDatabase.CreateAsset(timingDefinition, TestTurnTimingPath);
            }

            SerializedObject serializedTiming = WriteCommonContentFields(
                timingDefinition,
                FoundationTestSceneHarness.TestTurnTimingContentId,
                "地基测试回合时间换算",
            ...
            {
                throw new MissingReferenceException(
                    $"{nameof(TurnTimingDefinition)} 的每回合秒数字段已变更，测试规则生成器需要同步更新。");
            }
        ...
        private static void EnsureTestScenarioAsset()
        {
            ScenarioDefinition scenario =
                AssetDatabase.LoadAssetAtPath<ScenarioDefinition>(TestScenarioPath);
            if (scenario == null)
            {
                scenario = ScriptableObject.CreateInstance<ScenarioDefinition>();
                AssetDatabase.CreateAsset(scenario, TestScenarioPath);
            }
