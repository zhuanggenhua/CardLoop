...
namespace GameCore
{
    ...
    /// </summary>
    [InitializeOnLoad]
    public static class AIBridgePollerRecovery
    {
        private const double RetryDelaySeconds = 2.0;

        private static readonly string AIBridgeRoot =
            Path.Combine(Directory.GetParent(Application.dataPath)!.FullName, "Temp", "UnityBridge");

        private static readonly string CommandsDir = Path.Combine(AIBridgeRoot, "commands");
        private static readonly string HeartbeatFile = Path.Combine(AIBridgeRoot, "heartbeat");
        ...
        private static bool TryRebindBridgePoller()
        {
            ...
            {
                Debug.LogWarning("[AIBridgePollerRecovery] AIBridge editor types are unavailable. Recovery skipped.");
                return false;
            }
            ...
            {
                Debug.LogWarning($"[AIBridgePollerRecovery] Failed to rebind AIBridge poller: {exception.Message}");
                return false;
            }
        }
