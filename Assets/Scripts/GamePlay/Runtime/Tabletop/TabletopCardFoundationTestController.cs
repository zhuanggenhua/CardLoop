...
{
    /// <summary>
    /// `GamePlayFoundationTest` 场景的最小牌桌装配器。
    /// 它创建固定测试状态、绑定正式输入与视图，并把释放意图交给行动候选解析器；
    /// 释放接线不自动提交移动、拆堆、合堆或行动；验收代码可以通过显式选择入口启动普通行动作业。
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class TabletopCardFoundationTestController : MonoBehaviour
    {
        ...
        public GamePlayContentDiscoveryState DiscoveryState => m_discoveryState;
        ...
        private GamePlayContentDiscoveryState m_discoveryState;
        private GamePlayActionDefinition[] m_availableActions = Array.Empty<GamePlayActionDefinition>();
        ...
        private IEnumerator Start()
        {
            ...
            var contentId = new GamePlayContentId(TestContentId);
            if (!contentSystem.Index.TryGet(contentId, out GamePlayCardDefinition _))
            {
            ...
            if (!contentSystem.Index.TryGet(
                    new GamePlayContentId(TestActionContentId),
            ...
            if (!contentSystem.Index.TryGet(
                    new GamePlayContentId(TestProductContentId),
                    out GamePlayCardDefinition _))
            {
            ...
            m_discoveryState = new GamePlayContentDiscoveryState();
            m_discoveryState.MarkDiscovered(actionDefinition.ContentId, m_contentIndex);
            m_availableActions = GamePlayActionDiscoveryFilter.FilterDiscoveredActions(
        ...
        private void OnDisable()
        {
            ...
            m_availableActions = Array.Empty<GamePlayActionDefinition>();
        ...
        private static int[] GetTestAbilitySystemTags(GamePlayActionDefinition actionDefinition)
        {
        ...
        /// </summary>
        public TabletopCardActionJob StartSelectedAction(GamePlayContentId selectedActionId)
        {
            if (!TabletopCardActionCandidateSelection.TrySelect(
