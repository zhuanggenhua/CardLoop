...
namespace GamePlay
{
    ...
    /// 资源发现归 ResourceSystem，索引构建归 GamePlayContentIndex，本系统只负责二者的生命周期接入。
    /// </summary>
    public sealed class GamePlayContentSystem : AGameSystem
    {
        /// <summary>
        ...
        private ResourceHandle<IList<GamePlayContentAsset>> m_contentHandle;

        /// <summary>
        /// 当前进程已加载内容的只读运行时索引；系统关闭后恢复为空。
        /// </summary>
        public GamePlayContentIndex Index { get; private set; }
        ...
        public override void OnSystemInit()
        {
            ...
            ResourceHandle<IList<GamePlayContentAsset>> handle =
                ResourceSystem.LoadAssetsByAssetTagAsync<GamePlayContentAsset>(YooAssetContentTag);
            try
            {
                IList<GamePlayContentAsset> contentAssets = handle.WaitForCompletion();
                Index = GamePlayContentIndex.Build(contentAssets);
