...
namespace GamePlay
{
    ...
    /// </summary>
    public static class GamePlayActionDiscoveryFilter
    {
        /// <summary>
        /// 返回已被发现的行动集合，顺序保持调用方传入的当前可用行动顺序。
        /// 调用方仍然负责提供当前位置 / 世界规则下可用的行动候选，本方法只处理发现门槛。
        /// </summary>
        public static GamePlayActionDefinition[] FilterDiscoveredActions(
            IReadOnlyList<GamePlayActionDefinition> availableActions,
            GamePlayContentDiscoveryState discoveryState)
        {
            ...
            var discovered = new List<GamePlayActionDefinition>();
            for (int i = 0; i < availableActions.Count; i++)
            {
                GamePlayActionDefinition action = availableActions[i];
                if (action == null)
                {
