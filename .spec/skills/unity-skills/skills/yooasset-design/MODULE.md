...
description: 'YooAsset 设计规则入口。用于初始化、play mode、package、handle、加载、更新、filesystem、build 和坑点。'
---
...
- Asmdef & assembly layout for YooAsset consumers → load [asmdef](../asmdef/MODULE.md)
- Async orchestration across multiple YooAsset operations → load [async](../async/MODULE.md)
- Architecture-level decisions (Addressables vs YooAsset, single-package vs multi-package) → load [architecture](../architecture/MODULE.md)
- Performance review of load/release hot paths → load [performance](../performance/MODULE.md)
