...
description: 'DOTween 设计规则入口。用于 tween、Sequence、ease、shortcut、callback、SafeMode、生命周期和 UniTask/Addressables 集成。'
---
...
- Async integration (`ToUniTask`, `AsyncWaitForCompletion`) → load [unitask-design](../unitask-design/MODULE.md) and [async](../async/MODULE.md)
- UI animation (uGUI / UIToolkit) choice between DOTween and native animators → load [inspector](../inspector/MODULE.md)
- Architecture / performance review of tween-heavy scenes → load [performance](../performance/MODULE.md)
- Asmdef layout for DOTween consumers (`DOTween.asmdef`, DOTween Modules asmdef) → load [asmdef](../asmdef/MODULE.md)
- Addressables-loaded prefab tween lifetime (bundle unload vs tween running) → load [addressables-design](../addressables-design/MODULE.md)
