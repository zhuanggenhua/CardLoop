---
name: unity-yooasset
description: 'Unity YooAsset 操作入口。用于 bundle build、simulate build、Collector、BuildReport、热更资源和运行时验证。'
---
...
> **Strongly recommended**: before writing ANY YooAsset runtime code, load [yooasset-design](../yooasset-design/MODULE.md). PlayMode / parameter-class / handle-lifecycle pitfalls are strict, and only the advisory module surfaces them.
...
- Runtime code generation/design (Initialize / Load / Download / Scene in game scripts) → write it yourself using [yooasset-design](../yooasset-design/MODULE.md).
...
- Runtime-side rules (PlayMode mapping, handle lifecycle, update flow, legacy-API migration) → [yooasset-design/MODULE.md](../yooasset-design/MODULE.md).
