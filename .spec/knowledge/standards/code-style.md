...
- 自有玩法顶级模块统一拼作 `Gameplay`，不得新增 `Gameplay`。正式 C# 命名空间按真实职责分为 `Gameplay.Content`、`Gameplay.Actions`、`Gameplay.Tabletop`、`Gameplay.Scenarios`；编辑器工具在 `Gameplay.Editor.<职责>`，测试在 `Gameplay.Tests`。目录、程序集根命名空间和作者菜单必须同步使用这一拼写与层级。
